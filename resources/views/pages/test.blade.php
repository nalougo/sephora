...
<body class="min-h-screen flex items-center justify-center text-white bg-gray-900">
    ...
    <div class="w-full max-w-md p-4 shadow-lg bg-opacity-90 rounded-lg">
        ...
        <form action="{{ route('projection.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
            ...
            <div>
                ...
                <select name="code_film" id="code_film" class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg">
                    @foreach ($films as $film)
                        <option value="{{ $film->id }}" {{ old('code_film') == $film->id ? 'selected' : '' }}>{{ $film->titre }}</option>
                    @endforeach
                </select>
                @error('code_film')
            ...
            <div>
                ...
                <input type="date" id="date" name="date" class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg" value="{{ old('date') }}">
                @error('date')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="time" id="heure" name="heure" class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg" value="{{ old('heure') }}">
                @error('heure')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="text" id="lieu" name="lieu" class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg" placeholder="Entrez le lieu de projection" value="{{ old('lieu') }}">
                @error('lieu')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="file" id="image" name="image" class="w-full p-3 border border-gray-600 bg-gray-700 text-white rounded-lg" accept="image/*">
                @error('image')
                    <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
